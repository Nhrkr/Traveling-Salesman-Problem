...
def tsp_ga(max_generations=1000):
    init()
    global generation, offspring, current_popu
    generation+=1
...
n= int(input())
graph = [x[:] for x in [[0] * n] * n]
for i in range(n):
    graph[i] = list(map(int,input().split()))
    
start_city = int(input())
tsp_ga()
print("Path: " + str(current_popu[0].cities), "Cost: " + str(1 / fitness(current_popu[0])))
